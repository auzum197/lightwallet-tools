...
enum Cmd {
    ...
    /// Full mempool transactions. Ends at the next block unless --follow
    GetMempoolStream {
        /// Keep reconnecting across block boundaries instead of ending at the
        /// next block. Runs until Ctrl-C; txids already seen are suppressed, so
        /// the re-sent unmined txs don't repeat. Not available with --output
        /// debug (no parsed txid to dedup on).
        #[arg(long)]
        follow: bool,
        /// With --follow, stop after this many block boundaries (0 = unbounded)
...
/// clients rather than the indexers. lwcli is one process,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Cmd::GetTransaction { txid } => {
            let txid = Txid::new(parse_txid(&txid)?);
        ...
        Cmd::SendTransaction { tx } => {
            let data = TxBytes::new(parse_tx_hex(&tx)?);
            dispatch_identity!(variant, channel, |ix, proto| {
        ...
        Cmd::GetMempoolStream { follow, blocks } => dispatch!(variant, channel, |ix, proto| {
            if follow && output == OutputMode::Debug {
                bail!(
                    "--follow needs a parsed txid to suppress repeats, unavailable with --output debug"
                );
            }
            ...
            // each txid once for the life of the follow.
            let mut seen = std::collections::HashSet::new();
            let mut boundaries = 0u32;
            loop {
                let mut txs = ix.get_mempool_stream().await.map_err(rpc_err)?;
                while let Some(item) = txs.next().await {
                    let raw = item.map_err(rpc_err)?;
                    if output == OutputMode::Debug {
                    ...
                    match output {
                        OutputMode::Human => render::emit(&view.summary())?,
                        OutputMode::Json => render::emit(
                            &serde_json::to_string(&view).context("serializing tx view")?,
                        )?,
                        OutputMode::Debug => {} // handled above
                    }
                }
                // The stream ends at a block boundary. One-shot stops; --follow
                // reconnects for the next block unless the bound is reached.
                if !follow {
                    break;
                }
                boundaries += 1;
                if blocks != 0 && boundaries >= blocks {
                    break;
                }
            }
            Ok(())
