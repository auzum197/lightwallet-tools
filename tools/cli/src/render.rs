...
pub enum OutputMode {
    /// Parsed and readable. For RPCs with a transaction projection (the mempool
    /// stream), the interpreted view; elsewhere the same faithful JSON as `json`
    /// until those projections exist.
    Human,
    /// The parsed projection as JSON where one exists, else faithful proto JSON.
    /// Pretty for unary responses, NDJSON for stream items.
    Json,
    /// Rust `Debug` formatting of the generated types, the faithful wire view.
    Debug,
}
...
impl Renderer {
    ...
    /// A unary response: pretty JSON, or `{:#?}` in debug mode. Human falls back
    /// to faithful JSON for RPCs without a projection yet.
    pub fn unary<M: prost::Message + Debug>(&self, msg: &M, type_name: &str) -> Result<()> {
        match self.mode {
            OutputMode::Human | OutputMode::Json => {
                emit(&serde_json::to_string_pretty(&self.json(msg, type_name)?)?)
            }
    ...
    pub fn item<M: prost::Message + Debug>(&self, msg: &M, type_name: &str) -> Result<()> {
        match self.mode {
            OutputMode::Human | OutputMode::Json => {
                emit(&serde_json::to_string(&self.json(msg, type_name)?)?)
            }
            OutputMode::Debug => emit(&format!("{msg:?}")),
        }
    }
