...
lwcli --url http://127.0.0.1:9067 --variant crosslink get-roster
```

Flags: `--variant` picks canonical (default) or crosslink. Crosslink-only
commands (`get-roster`, `get-bond-info`, `request-faucet-donation`) require
an explicit `--variant crosslink` and hard-fail without it. `--transport`
picks direct (default), `tor` (in-process arti, bootstraps on first use), or
`nym` (through a running `nym-socks5-client`, address via `--nym-socks5`,
which selects nothing on its own). `--output` is `json` (default) or
`debug`. `completions <shell>` prints a completion script.
