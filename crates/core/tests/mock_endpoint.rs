...
async fn sync_to_tip<I: IndexerClient>(indexer: &I, start: u64) -> u64 {
...
async fn assert_tip_unavailable<I: IndexerClient>(indexer: &I) {
...
async fn assert_mid_stream_fault<I: IndexerClient>(
    indexer: &I,
    start: u64,
    end: u64,
...
async fn crosslink_indexer_runs_the_same_generic_sync() {
    ...
    let indexer = CrosslinkIndexerClient::new(crosslink::serve(mock).await, params("mock"));
...
async fn crosslink_empty_mock_reports_unavailable_at_the_tip() {
    let indexer = CrosslinkIndexerClient::new(
...
// The IndexerClient impls are hand-written per variant (only the streamer
