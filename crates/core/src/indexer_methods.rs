...
//! so it cannot leak onto the sync channel.
//!
...
macro_rules! impl_indexer_methods {
    ($indexer:ident, $proto:ident) => {
        impl<T: $crate::transport::GrpcTransport> $indexer<T> {
            ...
            > {
                let mut client = self.client.clone();
                let stream = client
                    .get_block_range_nullifiers($crate::request::block_range!($proto, start, end))
                    .await?
                    .into_inner();
                Ok($crate::error::wrap_stream(stream))
            }
...
pub(crate) use impl_indexer_methods;
