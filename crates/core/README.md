...
identical inherent methods on `CanonicalIndexerClient` and `CrosslinkIndexerClient`,
returning each variant's own generated types, with Crosslink-only RPCs
(roster, bond info) concrete on `CrosslinkIndexerClient`. Identity-bearing RPCs
